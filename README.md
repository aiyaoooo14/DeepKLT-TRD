...
第三方开源软件的**原样转存**备份仓，为 DeepKLT 主项目提供依赖兜底。

> **本仓只存放第三方开源依赖的原始源码，与 DeepKLT 主项目完全解耦。**
> DeepKLT 自有代码（SDK wrapper、Python adapter、业务逻辑等）不属于本仓范围。
...
4. **与主项目解耦**——只存第三方原始源码，不存 DeepKLT 自有代码。判断标准：假设上游仓库彻底消失，仅凭本仓内容能否从零重建该依赖；如果需要 DeepKLT 自有代码才能用，说明放错了地方。
...
| **Chrome for Testing 150.0.7871.46** | klt-browser 的 headless Chrome 后端（**绿色版，版本锁定，推荐替代系统 Chrome**） | Google ToS（二进制按 Google 服务条款分发） | https://googlechromelabs.github.io/chrome-for-testing/ | 本地备份 `src/chrome-for-testing-150.0.7871.46-linux64.zip`（⚠ 179MB 超 GitHub 100MB 限制，**不入 git**，通过 CDN 获取） | **推荐**：`klt-browser install`（从 CDN 自动下载到 `~/.agent-browser/browsers/`）；手动：`curl -L https://storage.googleapis.com/chrome-for-testing-public/150.0.7871.46/linux64/chrome-linux64.zip \| unzip -d ~/.agent-browser/browsers/chrome-150.0.7871.46` |
| browser-harness 0.1.4 | AI 浏览器自动化（视觉驱动，截图+坐标点击，browser-use 生态） | MIT | https://github.com/browser-use/browser-harness | `src/browser-harness-0.1.4.tar.gz`（已存 v0.1.4 / 2026-07-07） | DeepKLT 当前纯文本模型不适用（视觉驱动刚需视觉模型）；接入多模态模型后重新评估；当前仅作调研储备 |
...
└── src/
    ...
    ├── chrome-for-testing-150.0.7871.46-linux64.zip  # CFT 绿色版（推荐，klt-browser headless 后端）
    ├── firefox-devtools-mcp-0.9.9/                # Firefox 浏览器自动化（klt-browser-fox 上游）
    │   ├── src/                  # TypeScript 源码（注意：要 npm run build 后生成 dist/）
    │   ├── package.json
    │   ├── LICENSE-MIT / LICENSE-APACHE
    │   └── ...
    └── browser-harness-0.1.4.tar.gz               # 视觉驱动浏览器自动化（调研储备，当前不适用）
```
...
- **zip**（如 Chrome for Testing）：`unzip src/chrome-for-testing-*.zip -d ~/.agent-browser/browsers/chrome-<version>` → klt-browser 自动发现
