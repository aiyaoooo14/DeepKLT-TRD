...
| klt-browser-fox 0.1.0 | DeepKLT Firefox 浏览器 MCP SDK（Python wrapper + vendor 源码） | wrapper Apache-2.0 / vendor MIT OR Apache-2.0 | DeepKLT 主项目 `sdk/klt-browser-fox/` | `src/klt-browser-fox-0.1.0.tar.gz`（已存 v0.1.0 / 2026-07-06，含 wrapper + vendor 源码，不含 node_modules/dist） | `pip install src/klt-browser-fox-0.1.0.tar.gz`（或解压后 `pip install .`）；`cd vendor/firefox-devtools-mcp && npm install && npm run build` 构建 |
...
└── src/
    ...
    │   └── ...
    └── klt-browser-fox-0.1.0.tar.gz               # DeepKLT Firefox 浏览器 MCP SDK（含 vendor 源码）
```
...
- **SDK 包**（如 klt-browser-fox）：`pip install src/klt-browser-fox-0.1.0.tar.gz`
